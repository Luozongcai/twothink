...

{
    ...
    public function index(){
        ...
        $list=Db::name('service')->paginate(5);
       // var_dump($list);die;
        $this->assign('pid', $pid);
    ...
    public function edit($id = 0){
        if($this->request->isPost()){
            ...
            //获取父导航
            if(!empty($pid)){
                $parent = \think\Db::name('service')->where(array('id'=>$pid))->find();
    ...
    public function status()
    {
        ...
        $query = Db::name('service')->where(array('id'=>$id));

        if($query->find()['status'] == 1){
            Db::name('service')->where(array('id'=>$id))->update(['status'=>2]);
            $this->success('已禁用!');

        } else {
            Db::name('service')->where(array('id'=>$id))->update(['status'=>1]);
            $this->success('发布成功！');
        }
    }
